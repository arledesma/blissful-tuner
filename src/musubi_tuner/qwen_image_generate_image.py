...
def parse_args() -> argparse.Namespace:
    ...
    parser = argparse.ArgumentParser(description="Qwen-Image inference script")
    ...
    # parser.add_argument("--fp8_vl", action="store_true", help="use fp8 for Text Encoder (Qwen2.5-VL). Default is bfloat16")
...
def load_dit_model(
    ...
    Returns:
        qwen_image_model.QwenImageTransformer2DModel: DiT model instance
    """
...
def decode_latent(
    ...
    if enable_tiling:
        ...
        vae.disable_tiling()
    with torch.no_grad():
        pixels = vae.decode_to_pixels(latent.to(device))  # decode to pixels
...
def generate(
...
) -> tuple[Optional[AutoencoderKLQwenImage], torch.Tensor]:  # VAE can be Optional
    ...
    if shared_models is None or "model" not in shared_models:
        ...
        # use shared model
        model: qwen_image_model.QwenImageTransformer2DModel = shared_models["model"]
...
def save_output(
    ...
    if latent.ndim == 2:  # S,C. For packed latents from other inference scripts
        ...
        latent = qwen_image_utils.unpack_latents(latent, height, width)

    video = decode_latent(vae, latent, device, args.vae_enable_tiling)
